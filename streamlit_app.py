...
STARTRAIL_SEGMENTS = [
    {"num": "01", "ko": "성단", "en": "Star Cluster", "purpose": "팬덤 단위 이동이 가능한 집단형 후보 발굴", "criteria": "그룹/아이돌성, 팬덤 결집, 멤버 단위 이동 가능성", "point": "팬덤 단위 유입으로 CIME의 초기 트래픽을 확보합니다."},
    {"num": "02", "ko": "프로토스타", "en": "Protostar", "purpose": "외부 규모는 작지만 방송 반응이 좋은 성장형 후보 발굴", "criteria": "시청자 반응, 채팅, 뷰어십, 팔로워 대비 성과", "point": "성장 가능성이 높은 원석을 육성 타깃으로 확보합니다."},
    {"num": "03", "ko": "위성", "en": "Satellite", "purpose": "후원·채팅·시청자 반응이 검증된 개인 후보 발굴", "criteria": "도네이션, 채팅화력, 평균 시청자, 솔로 여부", "point": "검증된 개인 화력으로 수익화와 방송 활력을 만듭니다."},
    {"num": "04", "ko": "슈퍼노바", "en": "Supernova", "purpose": "대중성과 팬덤 체급이 큰 간판형 후보 발굴", "criteria": "팔로워, 최고 시청자, 유튜브 구독자, 팬덤지수, 방송화력", "point": "간판 스트리머로 인지도와 트래픽을 빠르게 끌어올립니다."},
    {"num": "05", "ko": "코멧", "en": "Comet", "purpose": "외부 채널 기반 신규 유입 후보 발굴", "criteria": "유튜브 구독자, X 팔로워, 유튜브/X 유입지수, 플랫폼 대비 외부 체급", "point": "외부 채널 기반으로 신규 유저층을 CIME로 유입시킵니다."},
]

STARSEED_STEPS = [
    {"num": "01", "title": "유튜브 기반 후보 발굴", "desc": "YouTube Data API를 활용해 CIME의 타깃 세그먼트와 맞는 채널과 영상을 수집합니다."},
    {"num": "02", "title": "팬덤 반응 분석", "desc": "조회수 대비 좋아요·댓글 반응을 확인하여 단순 노출이 아닌 팬덤 밀도를 판단합니다."},
    {"num": "03", "title": "라이브 전환 가능성 검토", "desc": "게임, 노래, ASMR, 토크 등 실시간 방송으로 확장 가능한 콘텐츠인지 확인합니다."},
    {"num": "04", "title": "영입 실전성 검증", "desc": "방송사, 기관, 팬클립, 아카이브, 리믹스 채널 등 실제 영입 대상이 아닌 채널을 제외합니다."},
    {"num": "05", "title": "Shortlist 생성", "desc": "최종점수, 액션버킷, 리스크 플래그를 함께 확인해 사람이 검토할 후보 리스트로 정리합니다."},
]
...
# =========================================================
# 1-3. CIME STREAM PLANET 홈/스타시드 네비게이션 스타일
# =========================================================
...
st.markdown(
    clean_html(
        ...
        .twinkle-star {
            ...
            pointer-events: none;
            z-index: 0;
            box-shadow: 0 0 10px rgba(255,255,255,0.95), 0 0 20px rgba(198,168,255,0.60);
        ...
        .twinkle-star:nth-child(3n) {
            background: #C8A8FF;
            box-shadow: 0 0 12px rgba(200,168,255,0.95), 0 0 25px rgba(139,92,255,0.58);
        }

        .twinkle-star:nth-child(5n) {
            background: #FF9DF5;
            box-shadow: 0 0 12px rgba(255,157,245,0.95), 0 0 25px rgba(255,120,230,0.50);
        }

        @keyframes twinkle {
            0%, 100% { opacity: 0.16; transform: scale(0.65); filter: brightness(0.75); }
            45% { opacity: var(--max-opacity); transform: scale(1.50); filter: brightness(1.7); }
            65% { opacity: 0.45; transform: scale(0.95); filter: brightness(1.05); }
        }
        ...
        section[data-testid="stSidebar"] .stButton > button[kind="primary"] {
            ...
            box-shadow: 0 0 18px rgba(125,66,255,0.25) !important;
        }
        ...
        section[data-testid="stSidebar"] .stButton > button:hover {
            transform: translateY(-1px);
            box-shadow: 0 0 18px rgba(125,66,255,0.32) !important;
        }
        ...
        .planet-orbit.orbit-2 { width: 660px; height: 185px; border-color: rgba(239,156,255,0.42); transform: rotate(1deg); }
        .planet-orbit.orbit-3 { width: 980px; height: 310px; border-color: rgba(125,66,255,0.20); transform: rotate(-4deg); }
        ...
        .planet {
            ...
            box-shadow: 0 0 34px rgba(246,166,255,0.62), 0 0 100px rgba(139,53,255,0.55), inset 22px 18px 48px rgba(255,170,255,0.20), inset -48px -44px 78px rgba(0,0,0,0.55);
            overflow: hidden;
        ...
        .planet-logo {
            ...
            letter-spacing: 5px;
            color: #FFF8FF;
            text-shadow: 0 0 10px rgba(255,255,255,0.85), 0 0 22px rgba(234,203,255,0.9), 0 0 40px rgba(179,93,255,0.8);
        }
        ...
        .mission-card {
            ...
            border: 1px solid rgba(255,255,255,0.08);
            box-shadow: 0 0 28px rgba(112, 53, 255, 0.18), inset 0 0 28px rgba(255,255,255,0.025);
        ...
        .trail-card::before { background: #FFD45D; box-shadow: 0 0 16px rgba(255,212,93,0.55); }
        .seed-card::before { background: #98FFAB; box-shadow: 0 0 16px rgba(152,255,171,0.45); }
        .card-head { display: flex; align-items: center; gap: 24px; margin-top: 20px; margin-bottom: 26px; }
        .icon-box { width: 66px; height: 66px; border-radius: 18px; display: flex; align-items: center; justify-content: center; }
        .trail-icon-box { background: rgba(255,212,93,0.12); border: 1px solid rgba(255,212,93,0.34); }
        .seed-icon-box { background: rgba(152,255,171,0.10); border: 1px solid rgba(152,255,171,0.30); }
        .card-title { font-size: 40px; font-weight: 950; color: #FFF9FF; line-height: 1.05; }
        .card-en { font-size: 27px; font-weight: 900; margin-top: 6px; }
        .trail-en { color: #FFD45D; }
        .seed-en { color: #98FFAB; }
        .card-desc { font-size: 19px; line-height: 1.7; color: #D9CFE8; font-weight: 560; }

        .css-star { width: 0; height: 0; color: #FFD45D; position: relative; display: block; border-right: 16px solid transparent; border-bottom: 11px solid #FFD45D; border-left: 16px solid transparent; transform: rotate(35deg); filter: drop-shadow(0 0 8px rgba(255,212,93,0.7)); }
        .css-star::before { border-bottom: 13px solid #FFD45D; border-left: 5px solid transparent; border-right: 5px solid transparent; position: absolute; height: 0; width: 0; top: -9px; left: -10px; display: block; content: ""; transform: rotate(-35deg); }
        .css-star::after { position: absolute; display: block; color: #FFD45D; top: 1px; left: -17px; width: 0; height: 0; border-right: 16px solid transparent; border-bottom: 11px solid #FFD45D; border-left: 16px solid transparent; transform: rotate(-70deg); content: ""; }
        .css-seed { position: relative; width: 42px; height: 50px; }
        .css-seed::before { content: ""; position: absolute; left: 19px; top: 18px; width: 4px; height: 28px; background: #98FFAB; border-radius: 4px; box-shadow: 0 0 10px rgba(152,255,171,0.6); }
        .leaf-left, .leaf-right { position: absolute; background: #98FFAB; box-shadow: 0 0 10px rgba(152,255,171,0.55); }
        .leaf-left { width: 28px; height: 18px; left: 1px; top: 7px; border-radius: 28px 4px 28px 4px; transform: rotate(20deg); }
        .leaf-right { width: 30px; height: 19px; right: 0; top: 4px; border-radius: 4px 28px 4px 28px; transform: rotate(-20deg); }
        .seed-pot { position: absolute; left: 13px; top: 37px; width: 18px; height: 12px; background: #A97767; border-radius: 0 0 12px 12px; }

        .home-card-button .stButton > button,
        .starseed-launch .stButton > button {
            width: 100%;
            height: 44px;
            border-radius: 15px !important;
        ...
        .detail-box {
            margin-top: 28px;
            border-radius: 24px;
            background: rgba(20, 15, 45, 0.88);
            border: 1px solid rgba(255,255,255,0.10);
            padding: 30px 34px;
            box-shadow: 0 0 30px rgba(125,66,255,0.16);
            text-align: left;
        }

        .detail-kicker { color: #C8A8FF; font-weight: 850; letter-spacing: 1.8px; font-size: 12px; margin-bottom: 10px; }
        .detail-title { font-size: 28px; font-weight: 950; color: #FFF9FF; margin-bottom: 12px; }
        .detail-text { font-size: 16px; line-height: 1.75; color: #D9CFE8; margin-bottom: 24px; }
        .segment-grid, .seed-grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 14px; }
        .segment-card, .seed-step-card { border-radius: 18px; background: rgba(255,255,255,0.045); border: 1px solid rgba(255,255,255,0.10); padding: 18px; min-height: 180px; }
        .segment-num, .seed-num { color: #C8A8FF; font-weight: 900; font-size: 12px; margin-bottom: 8px; }
        .segment-ko, .seed-title { color: #FFF9FF; font-weight: 900; font-size: 17px; margin-bottom: 4px; }
        .segment-en { color: #D9C8FF; font-weight: 800; font-size: 13px; margin-bottom: 12px; }
        .segment-purpose, .seed-desc { color: #D9CFE8; font-size: 13px; line-height: 1.55; }
        .field-label { color: #B99BFF; font-size: 11px; font-weight: 850; margin-top: 12px; margin-bottom: 4px; }
        ...
        @media (max-width: 1100px) {
            ...
            .segment-grid, .seed-grid { grid-template-columns: repeat(2, 1fr); }
        }

        @media (max-width: 780px) {
            .segment-grid, .seed-grid { grid-template-columns: 1fr; }
        }
        </style>
        """
    ),
...
def render_mission_cards():
    ...
    with col1:
        html(
            ...
            <div class="card-head"><div class="icon-box trail-icon-box"><div class="css-star"></div></div>
            <div><div class="card-title">스타트레일</div><div class="card-en trail-en">Star Trail</div></div></div>
            <div class="card-desc">기존 플랫폼에서 활동성과 팬덤이 확인된<br>스트리머 영입 후보군을 발굴합니다.</div>
            </div>
            """
        )
    ...
    with col2:
        html(
            ...
            <div class="card-head"><div class="icon-box seed-icon-box"><div class="css-seed"><div class="leaf-left"></div><div class="leaf-right"></div><div class="seed-pot"></div></div></div>
            <div><div class="card-title">스타시드</div><div class="card-en seed-en">Star Seed</div></div></div>
            <div class="card-desc">유튜브 기반 잠재 후보의 팬덤 반응과<br>라이브 전환 가능성을 분석합니다.</div>
            </div>
            """
        )
...
def render_startrail_detail():
    cards = "".join(
        [
            f'<div class="segment-card"><div class="segment-num">{seg["num"]}</div><div class="segment-ko">{seg["ko"]}</div><div class="segment-en">{seg["en"]}</div><div class="segment-purpose">{seg["purpose"]}</div><div class="field-label">주요 판단 기준</div><div class="field-value">{seg["criteria"]}</div><div class="field-label">CIME 활용 포인트</div><div class="field-value">{seg["point"]}</div></div>'
            for seg in STARTRAIL_SEGMENTS
        ]
    )
    st.markdown(
        '<div class="detail-box"><div class="detail-kicker">PHASE 01 · STAR TRAIL</div><div class="detail-title">기존 플랫폼의 데이터 궤적을 따라 영입 후보군을 찾는 단계</div><div class="detail-text">스타트레일은 기존 플랫폼에서 이미 빛을 내고 있는 스트리머들의 데이터 흐름을 따라, CIME로 유입 가능한 영입 후보군을 발굴하는 1단계 분석입니다.</div>'
...
def render_starseed_detail():
    cards = "".join(
        [
            f'<div class="seed-step-card"><div class="seed-num">{step["num"]}</div><div class="seed-title">{step["title"]}</div><div class="seed-desc">{step["desc"]}</div></div>'
            for step in STARSEED_STEPS
        ]
    )
    st.markdown(
        '<div class="detail-box"><div class="detail-kicker">PHASE 02 · STAR SEED</div><div class="detail-title">유튜브 기반 잠재 후보를 발굴해 라이브 전환 가능성을 보는 단계</div><div class="detail-text">스타시드는 유튜브에서 활동 중인 크리에이터 중 CIME에서 라이브 스트리머로 성장할 가능성이 있는 잠재 후보를 발굴하는 2단계 분석입니다.</div>'
...
if st.session_state.page == "대시보드 홈":
    ...
    html(
        ...
        <div class="page-panel">이 영역에는 추후 스타트레일 분석 대시보드가 들어갈 예정입니다.</div>
        """
    )
    st.stop()
    ...
    with st.container(border=True):
        ...
        st.markdown(
            ...
            <div class="guide-box">
                ...
                점수는 높지만 참여율이나 조회수 규모가 낮은 후보는 수기 검증이 필요하고, 점수·참여율·라이브친화 신호가 함께 높은 후보는 우선 컨택 후보로 볼 수 있습니다.
            </div>
            """,
            unsafe_allow_html=True,
        )
