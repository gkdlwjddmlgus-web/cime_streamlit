...
st.markdown(
    clean_html(
        ...
        /* sidebar toggle style is injected once at the end of the file. */
...
.trail-top5-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(132px,1fr)); gap:16px; align-items:stretch; }
.trail-rank-card {
...
/* =========================================================
   STARTRAIL GOLD THEME: align Star Trail dashboard to gold/yellow system
========================================================= */
.startrail-page {
    --trail-gold: #FFD45D;
    --trail-gold-soft: #FFE9A8;
    --trail-gold-deep: #B77A18;
    --trail-panel: rgba(18, 16, 34, .88);
    --trail-panel-strong: rgba(23, 18, 36, .94);
    --trail-border: rgba(255, 212, 93, .24);
}
.trail-title {
    color: #FFF9E8 !important;
    text-shadow: 0 0 8px rgba(255,255,255,.52), 0 0 26px rgba(255,212,93,.46), 0 0 62px rgba(183,122,24,.36) !important;
}
.trail-subtitle,
.trail-info-desc,
.trail-kpi-label,
.trail-seg-desc {
    color: rgba(247,235,202,.78) !important;
}
.trail-date-pill {
    color: #FFF1BC !important;
    background: rgba(34, 25, 26, .78) !important;
    border-color: rgba(255,212,93,.42) !important;
    box-shadow: 0 0 18px rgba(255,212,93,.16), inset 0 1px 0 rgba(255,255,255,.08) !important;
}
.trail-info-card,
.trail-kpi-card,
.trail-seg-card,
...
.trail-chart-card,
div[data-testid="stPlotlyChart"] {
    border-color: var(--trail-border) !important;
    background: radial-gradient(circle at 11% 26%, rgba(255,212,93,.090), transparent 34%), linear-gradient(180deg, var(--trail-panel-strong), rgba(8,8,25,.96)) !important;
    box-shadow: inset 0 1px 0 rgba(255,255,255,.045), 0 14px 30px rgba(0,0,0,.22), 0 0 24px rgba(255,212,93,.09) !important;
}
.trail-info-icon,
.trail-seg-icon-badge,
.trail-detail-avatar {
    background: radial-gradient(circle at 34% 26%, #FFF9D8, #FFD45D 48%, #B77A18 100%) !important;
    border-color: rgba(255,226,135,.48) !important;
    box-shadow: 0 0 18px rgba(255,212,93,.22), inset 0 0 18px rgba(255,255,255,.08) !important;
}
.trail-seg-emoji {
...
.trail-divider,
.trail-seg-card::before {
    background: linear-gradient(90deg, rgba(255,212,93,0), rgba(255,212,93,.72), rgba(255,241,188,.92), rgba(255,212,93,.38), rgba(255,212,93,0)) !important;
    box-shadow: 0 0 18px rgba(255,212,93,.18) !important;
}
.trail-seg-card.active,
.trail-seg-card.active-성단,
.trail-seg-card.active-프로토스타,
.trail-seg-card.active-위성,
.trail-seg-card.active-슈퍼노바,
.trail-seg-card.active-코멧 {
    border-color: rgba(255,212,93,.82) !important;
    box-shadow: 0 0 28px rgba(255,212,93,.22), inset 0 0 24px rgba(255,212,93,.055) !important;
}
.trail-seg-card.active::before,
.trail-seg-card.active-성단::before,
.trail-seg-card.active-프로토스타::before,
.trail-seg-card.active-위성::before,
.trail-seg-card.active-슈퍼노바::before,
.trail-seg-card.active-코멧::before {
    background: var(--trail-gold) !important;
    box-shadow: 0 0 18px rgba(255,212,93,.66) !important;
}
.trail-seg-card.active-성단 .trail-seg-name,
.trail-seg-card.active-성단 .trail-seg-count,
.trail-seg-card.active-프로토스타 .trail-seg-name,
.trail-seg-card.active-프로토스타 .trail-seg-count,
.trail-seg-card.active-위성 .trail-seg-name,
.trail-seg-card.active-위성 .trail-seg-count,
.trail-seg-card.active-슈퍼노바 .trail-seg-name,
.trail-seg-card.active-슈퍼노바 .trail-seg-count,
.trail-seg-card.active-코멧 .trail-seg-name,
.trail-seg-card.active-코멧 .trail-seg-count {
    color: var(--trail-gold) !important;
}
.trail-rank-badge,
.trail-rank-badge.rank-1 {
    color: #211609 !important;
    background: linear-gradient(135deg, #FFF0A8 0%, #FFD45D 48%, #B77A18 100%) !important;
    border: 1px solid rgba(255,239,180,.72) !important;
    box-shadow: 0 0 16px rgba(255,212,93,.42) !important;
}
.trail-rank-badge.rank-2 {
    color: #171724 !important;
    background: linear-gradient(135deg, #FFFFFF 0%, #DDE3EF 48%, #8D98AA 100%) !important;
    border: 1px solid rgba(245,248,255,.72) !important;
    box-shadow: 0 0 15px rgba(221,227,239,.32) !important;
}
.trail-rank-badge.rank-3 {
    color: #FFF7EF !important;
    background: linear-gradient(135deg, #E3A86F 0%, #B8733A 52%, #734421 100%) !important;
    border: 1px solid rgba(232,176,119,.62) !important;
    box-shadow: 0 0 15px rgba(184,115,58,.30) !important;
}
.trail-rank-badge.rank-4,
.trail-rank-badge.rank-5,
.trail-rank-badge.rank-normal {
    color: #F6F1FF !important;
    background: linear-gradient(135deg, rgba(255,255,255,.20), rgba(126,118,148,.24)) !important;
    border: 1px solid rgba(255,255,255,.28) !important;
    box-shadow: 0 0 12px rgba(255,255,255,.10) !important;
}
.trail-avatar-wrap {
    background: radial-gradient(circle at 32% 28%, #fff, #FFD45D 42%, #4A2E0C 100%) !important;
    border-color: rgba(255,212,93,.54) !important;
    box-shadow: 0 0 18px rgba(255,212,93,.22) !important;
}
.trail-tag,
.trail-platform-text {
...
.trail-table th {
    ...
    border-bottom-color: rgba(255,212,93,.18) !important;
}
.trail-table td {
    border-bottom-color: rgba(255,212,93,.075) !important;
}
div[data-testid="column"]:has(.trail-detail-button-title) .stButton > button {
    color: #FFF3C5 !important;
    background: rgba(34,25,26,.86) !important;
    border-color: rgba(255,212,93,.35) !important;
    box-shadow: 0 0 12px rgba(255,212,93,.10) !important;
}
...
# =========================================================
# FINAL SIDEBAR TOGGLE PATCH - SINGLE SOURCE OF TRUTH
# - 홈/스타트레일/스타시드 전체 토글 디자인 통일
# - 부모 컨테이너만 원형 디자인 적용
# - 내부 button은 투명 클릭 레이어로만 사용
# - 스타시드에서 닫기 토글이 이중 스타일링되는 문제 방지
# =========================================================
st.markdown(
    """
    <style>
    :root {
        ...
        --cime-toggle-size: 42px;
        --cime-toggle-top: 22px;
        --cime-toggle-open-left: 18px;
        --cime-toggle-close-right-gap: 18px;

        --cime-toggle-bg: rgba(7, 22, 38, 0.94);
        --cime-toggle-border: rgba(118, 242, 226, 0.66);
    ...
    /* =====================================================
       2) 열린 상태: 사이드바 닫기 토글 컨테이너
       ===================================================== */
    html body .stApp section[data-testid="stSidebar"] [data-testid="stSidebarCollapseButton"] {
        position: fixed !important;
        top: var(--cime-toggle-top) !important;
        left: calc(
            var(--cime-sidebar-width)
            - var(--cime-toggle-size)
            - var(--cime-toggle-close-right-gap)
        ) !important;
    ...
    /* =====================================================
       3) Streamlit 버전에 따라 닫기 버튼이 wrapper 없이 button으로만 잡힐 때의 fallback
       - 단, wrapper가 있는 경우에는 위 규칙이 우선 작동
       ===================================================== */
    html body .stApp section[data-testid="stSidebar"] > div button[aria-label="Close sidebar"],
    html body .stApp section[data-testid="stSidebar"] > div button[title="Close sidebar"],
    html body .stApp section[data-testid="stSidebar"] > div button[aria-label="사이드바 닫기"],
    html body .stApp section[data-testid="stSidebar"] > div button[title="사이드바 닫기"] {
        position: fixed !important;
        top: var(--cime-toggle-top) !important;
        left: calc(
            var(--cime-sidebar-width)
    ...
    /* =====================================================
       4) 내부 button은 투명 클릭 레이어로만 사용
       - 여기서 background/border/box-shadow를 다시 주면 이중 버튼처럼 보임
       ===================================================== */
    ...
    html body .stApp [data-testid="collapsedControl"] > button svg,
    html body .stApp [data-testid="stSidebarCollapsedControl"] > button svg,
    html body .stApp section[data-testid="stSidebar"] [data-testid="stSidebarCollapseButton"] svg,
    html body .stApp section[data-testid="stSidebar"] [data-testid="stSidebarCollapseButton"] > button svg,
    ...
    /* fallback: wrapper 없이 button만 있는 경우 */
    html body .stApp section[data-testid="stSidebar"] > div button[aria-label="Close sidebar"]::before,
    html body .stApp section[data-testid="stSidebar"] > div button[title="Close sidebar"]::before,
    html body .stApp section[data-testid="stSidebar"] > div button[aria-label="사이드바 닫기"]::before,
    html body .stApp section[data-testid="stSidebar"] > div button[title="사이드바 닫기"]::before {
        content: "««" !important;
        position: absolute !important;
        inset: 0 !important;
    ...
    html body .stApp section[data-testid="stSidebar"] > div button[aria-label="Close sidebar"]:hover,
    html body .stApp section[data-testid="stSidebar"] > div button[title="Close sidebar"]:hover {
