...
def render_startrail_dashboard():
    ...
    def _metric_bar(label, value, max_value, color):
        ...
        display_value = (
            _format_krw_compact(_donation_to_krw(value))
            if "도네이션" in label
            else f"{value:,.0f}"
        )

        tooltip_map = {
            "뷰어십": "2025.01.01 ~ 2026.03.31 누적 집계",
            "도네이션": "2025.01.01 ~ 2026.03.31 누적 집계",
        }

        tooltip_html = ""
        for key, desc in tooltip_map.items():
            if key in label:
                tooltip_html = f'''
                <span class="trail-tooltip-wrap">
                    <span class="trail-tooltip-icon">?</span>
                    <span class="trail-tooltip-text">{html_lib.escape(desc)}</span>
                </span>
                '''
                break
        ...
        <div class="trail-bar-row">
            ...
            <div class="trail-bar-track">
                <div class="trail-bar-fill" style="width:{percent:.2f}%; background:{color};"></div>
            </div>
        </div>
        '''
            ...
            <div class="trail-kpi-grid">
                <div class="trail-kpi-card">
                    <div style="display:flex; align-items:center; gap:16px;">
                        <div style="width:54px; height:54px; border-radius:50%; display:flex; align-items:center; justify-content:center; background:radial-gradient(circle at 35% 25%, rgba(255,255,255,0.55), rgba(255,212,93,0.85) 42%, rgba(118,74,22,0.95) 100%); border:1px solid rgba(255,232,150,0.65); box-shadow:0 0 16px rgba(255,212,93,0.24); overflow:hidden;">
                            <img src="{startrail_local_image_src('assets/사람들.png')}" style="width:38px; height:38px; object-fit:contain; display:block;">
                        </div>
                        <div>
                ...
                <div class="trail-kpi-card">
                    <div style="display:flex; align-items:center; gap:16px;">
                        <div style="width:54px; height:54px; border-radius:50%; display:flex; align-items:center; justify-content:center; background:radial-gradient(circle at 35% 25%, rgba(255,255,255,0.55), rgba(255,212,93,0.85) 42%, rgba(118,74,22,0.95) 100%); border:1px solid rgba(255,232,150,0.65); box-shadow:0 0 16px rgba(255,212,93,0.24); overflow:hidden;">
                            <img src="{startrail_local_image_src('assets/눈1.png')}" style="width:32px; height:32px; object-fit:contain; display:block;">
                        </div>
                        <div>
               ...
               <div class="trail-kpi-card">
                    <div style="display:flex; align-items:center; gap:16px;">
                        <div style="width:54px; height:54px; min-width:54px; border-radius:50%; display:flex; align-items:center; justify-content:center; background:radial-gradient(circle at 35% 25%, #fff6c7 0%, #ffe27a 34%, #f4c542 62%, #b57917 100%); border:1px solid rgba(255,238,170,0.95); box-shadow:0 0 18px rgba(255,212,93,0.32), inset 0 1px 0 rgba(255,255,255,0.45); overflow:hidden;">
                            <img src="{startrail_local_image_src('assets/돈.png')}" style="width:22px; height:22px; object-fit:contain; display:block;">
                        </div>
                        <div>
                            <div class="trail-kpi-label">평균 도네이션(원화)</div>
                            <div class="trail-kpi-value">{_format_krw_compact(_donation_to_krw(avg_donation))}</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    st.markdown("""
        <style>
        /* 스타트레일 KPI 동그라미 아이콘 3개를 불투명 노란색으로 고정 */
        html body .stApp .startrail-page .trail-kpi-card > div > div:first-child {
            background:
            ...
            opacity: 1 !important;
            border: 1px solid rgba(255, 238, 170, 0.95) !important;
            box-shadow:
    ...
    with left_area:
        if top_5.empty:
            ...
            for i, (_, row) in enumerate(top_5.iterrows()):
                ...
                tag_style_map = {
                    "성단": "background:linear-gradient(135deg,#FFD45D,#B57917) !important; border:1px solid rgba(255,232,150,0.75) !important; color:#FFF9E8 !important;",
                    "프로토스타": "background:linear-gradient(135deg,#98FFAB,#1C9150) !important; border:1px solid rgba(152,255,171,0.65) !important; color:#F5FFF7 !important;",
                    "위성": "background:linear-gradient(135deg,#8FB8FF,#395BBE) !important; border:1px solid rgba(143,184,255,0.65) !important; color:#F5F9FF !important;",
                    "슈퍼노바": "background:linear-gradient(135deg,#FF7AC8,#AC317E) !important; border:1px solid rgba(255,157,245,0.65) !important; color:#FFF5FD !important;",
                    "코멧": "background:linear-gradient(135deg,#FF9E5E,#BC5823) !important; border:1px solid rgba(255,190,126,0.68) !important; color:#FFF7EF !important;",
                }

                tag_style = tag_style_map.get(display_segment, "")
                ...
                with card_cols[i]:
                    ...
                    <div class="trail-rank-card">
                        ...
                        <div>
                            <span class="trail-tag trail-tag-{html_lib.escape(display_segment)}">
                                {html_lib.escape(display_segment)}
                            </span>
                        </div>
    ...
    with right_area:
        if selected is not None:
            ...
            <div class="trail-side-card">
                ...
                <div class="trail-detail-tags"><span class="trail-tag trail-tag-{html_lib.escape(detail_seg)}">{html_lib.escape(detail_seg)}</span>{platform_html}</div>
                {bar_html}
            </div>
            ''')
            ...
            if filtered_df.empty:
                ...
                if missing:
                    ...
                    if proto_plot_df.empty:
                        ...
                        fig.update_layout(
                            title=dict(text="팔로워 대비 평균 시청자", x=0.05, xanchor="left"),
                            xaxis_title="최고 팔로워",
...
<div class="starseed-board">
    ...
    <div class="board-kpi-grid">
        <div class="board-kpi-card">
            <div class="board-kpi-icon">
                <img src="{startrail_local_image_src('assets/사람들.png')}"
                    style="width:30px;height:30px;object-fit:contain;display:block;">
            </div>
            <div>
                {kpi_tooltip_label("전체 분석 후보", kpi_tooltips["전체 분석 후보"])}
                <div class="board-kpi-value">{fmt_num(target_all_kpi['total'], 0, '명')}</div>
                {delta_badge(delta_total, 0, '명')}
            </div>
        </div>
        <div class="board-kpi-card">
    <div class="board-kpi-icon">
            <img src="{startrail_local_image_src('assets/흰깔.png')}"
                style="width:48px;height:48px;object-fit:contain;display:block;">
        </div>
        <div>
            {kpi_tooltip_label("1차 선별 후보", kpi_tooltips["1차 선별 후보"])}
            <div class="board-kpi-value">{fmt_num(target_all_kpi['shortlist'], 0, '명')}</div>
            {delta_badge(delta_shortlist, 0, '명')}
        </div>
    </div>
        ...
        <div class="board-kpi-card">
    <div class="board-kpi-icon">
            <img src="{startrail_local_image_src('assets/흰체크.png')}"
                style="width:32px;height:32px;object-fit:contain;display:block;">
        </div>
        <div>
            {kpi_tooltip_label("즉시 검토 후보", kpi_tooltips["즉시 검토 후보"])}
            <div class="board-kpi-value">{fmt_num(target_filtered_kpi['high_priority'], 0, '명')}</div>
            {delta_badge(delta_high_priority, 0, '명')}
        </div>
    </div>
    </div>
</div>
...
# =========================================================
# STAR SEED - 영입 우선순위 TOP 표 스타일 수정
# =========================================================
st.markdown(
    """
    <style>
    /* 스트리머명 가운데 정렬 */
    html body .stApp .priority-table td.name {
        text-align: center !important;
    }

    /* 주요 콘텐츠군 볼드 제거 */
    html body .stApp .priority-table td:nth-child(3),
    html body .stApp .priority-table td:nth-child(3) .tag-pill {
...
# =========================================================
# STAR SEED - KPI 아이콘 원형 배경 불투명 초록 처리
# =========================================================
st.markdown(
    """
    <style>
    html body .stApp .board-kpi-icon {
        background:
            radial-gradient(circle at 35% 25%,
                #EFFFF4 0%,
                #58E984 28%,
        ...
        box-shadow:
            0 0 12px rgba(80, 255, 146, 0.20),
            inset 0 1px 0 rgba(255,255,255,0.32) !important;
    }

    html body .stApp .board-kpi-icon img {
