...
# =========================================================
# 2-B. Star Trail 전용 CSS (팀원 파일 통합)
# =========================================================
st.markdown(dedent("""
...
.stMetric {
    ...
    color: #FFF9FF;
    box-shadow: 0 0 24px rgba(112, 53, 255, 0.16), inset 0 0 24px rgba(255,255,255,0.02);
}
[data-testid="stMetricLabel"] p { color: #D9C8FF !important; font-weight: 850; }
[data-testid="stMetricValue"] { color: #FFF9FF !important; text-shadow: 0 0 14px rgba(230,195,255,0.20); }
...
.segment-card {
    background: rgba(21, 16, 47, 0.90); border: 1px solid rgba(255,255,255,0.10);
    border-radius: 22px; padding: 22px 16px; transition: 0.25s ease;
    display: flex; flex-direction: column; align-items: center; text-align: center;
    min-height: 205px; box-shadow: 0 0 24px rgba(112,53,255,0.14), inset 0 0 24px rgba(255,255,255,0.018);
    overflow: hidden; position: relative;
}
.segment-card::before { content: ""; position: absolute; left: 22px; right: 22px; top: 16px; height: 3px; border-radius: 3px; background: rgba(240,140,255,0.36); box-shadow: 0 0 14px rgba(240,140,255,0.28); }
.segment-card.active { border-color: rgba(240,140,255,0.72); box-shadow: 0 0 28px rgba(125,66,255,0.30), inset 0 0 30px rgba(255,255,255,0.025); transform: translateY(-2px); }
.segment-card.active-성단 { border-color: #FF6B8A !important; box-shadow: 0 0 22px rgba(255,107,138,0.45), inset 0 0 24px rgba(255,107,138,0.08) !important; }
.segment-card.active-성단 i, .segment-card.active-성단 .seg-name, .segment-card.active-성단 .seg-count { color: #FF6B8A !important; }
.segment-card.active-프로토스타 { border-color: #FFD45D !important; box-shadow: 0 0 22px rgba(255,212,93,0.42), inset 0 0 24px rgba(255,212,93,0.08) !important; }
.segment-card.active-프로토스타 i, .segment-card.active-프로토스타 .seg-name, .segment-card.active-프로토스타 .seg-count { color: #FFD45D !important; }
.segment-card.active-위성 { border-color: #75CCFF !important; box-shadow: 0 0 22px rgba(117,204,255,0.42), inset 0 0 24px rgba(117,204,255,0.08) !important; }
.segment-card.active-위성 i, .segment-card.active-위성 .seg-name, .segment-card.active-위성 .seg-count { color: #75CCFF !important; }
.segment-card.active-슈퍼노바 { border-color: #98FFAB !important; box-shadow: 0 0 22px rgba(152,255,171,0.42), inset 0 0 24px rgba(152,255,171,0.08) !important; }
.segment-card.active-슈퍼노바 i, .segment-card.active-슈퍼노바 .seg-name, .segment-card.active-슈퍼노바 .seg-count { color: #98FFAB !important; }
.segment-card.active-코멧 { border-color: #FF9DF5 !important; box-shadow: 0 0 22px rgba(255,157,245,0.45), inset 0 0 24px rgba(255,157,245,0.08) !important; }
.segment-card.active-코멧 i, .segment-card.active-코멧 .seg-name, .segment-card.active-코멧 .seg-count { color: #FF9DF5 !important; }
.segment-card.active-성단::before { background: #FF6B8A !important; box-shadow: 0 0 18px rgba(255,107,138,0.65) !important; }
.segment-card.active-프로토스타::before { background: #FFD45D !important; box-shadow: 0 0 18px rgba(255,212,93,0.65) !important; }
.segment-card.active-위성::before { background: #75CCFF !important; box-shadow: 0 0 18px rgba(117,204,255,0.65) !important; }
.segment-card.active-슈퍼노바::before { background: #98FFAB !important; box-shadow: 0 0 18px rgba(152,255,171,0.65) !important; }
.segment-card.active-코멧::before { background: #FF9DF5 !important; box-shadow: 0 0 18px rgba(255,157,245,0.65) !important; }
.segment-card i { margin-top: 16px; margin-bottom: 13px; color: #DDBBFF; text-shadow: 0 0 14px rgba(221,187,255,0.30); }
.segment-card.active i { color: #FF9DF5; text-shadow: 0 0 18px rgba(255,157,245,0.55); }
.seg-name { font-size: 19px; font-weight: 950; margin-bottom: 7px; color: #FFF9FF; }
.seg-count { font-size: 25px; font-weight: 950; color: #F08CFF; margin-bottom: 9px; }
.seg-desc { font-size: 12px; color: rgba(255,255,255,0.70); line-height: 1.55; white-space: pre-wrap; word-break: keep-all; }

.rank-card { background: rgba(21,16,47,0.90); border-radius: 24px; padding: 22px 18px; text-align: center; border: 1px solid rgba(255,255,255,0.10); position: relative; transition: 0.25s ease; min-height: 260px; box-shadow: 0 0 24px rgba(112,53,255,0.14), inset 0 0 24px rgba(255,255,255,0.018); }
.rank-card:hover { border-color: rgba(240,140,255,0.72); box-shadow: 0 0 26px rgba(125,66,255,0.28); transform: translateY(-2px); }
.rank-badge { position: absolute; top: 15px; left: 15px; width: 34px; height: 34px; padding: 0; border-radius: 11px; display: flex; align-items: center; justify-content: center; font-weight: 950; font-size: 18px; }
.rank-normal { background: rgba(255,255,255,0.09); color: #FFF9FF; border: 1px solid rgba(255,255,255,0.14); }
.rank-gold { background: linear-gradient(135deg, #FFD45D, #C99700); color: #1a112f; box-shadow: 0 0 16px rgba(255,212,93,0.55); }
.rank-silver { background: linear-gradient(135deg, #F4E9FF, #B7A2DC); color: #1a112f; box-shadow: 0 0 16px rgba(229,213,255,0.45); }
.rank-bronze { background: linear-gradient(135deg, #D99666, #8B4A30); color: #ffffff; box-shadow: 0 0 16px rgba(205,127,50,0.45); }
.avatar-circle { width: 100px; height: 100px; border-radius: 50%; margin: 0 auto 15px; border: 3px solid rgba(240,140,255,0.28); background: #09051C; overflow: hidden; box-shadow: 0 0 18px rgba(240,140,255,0.16); }
.avatar-circle img { width: 100%; height: 100%; object-fit: cover; }

.tag-segment { display: inline-block; padding: 4px 10px; border-radius: 999px; font-size: 13px; font-weight: 700; margin-bottom: 10px; }
.tag-성단 { background: rgba(255,107,138,0.20); color: #FF6B8A; border: 1px solid rgba(255,107,138,0.40); }
.tag-프로토스타 { background: rgba(255,212,93,0.18); color: #FFD45D; border: 1px solid rgba(255,212,93,0.38); }
.tag-위성 { background: rgba(117,204,255,0.18); color: #75CCFF; border: 1px solid rgba(117,204,255,0.38); }
.tag-슈퍼노바 { background: rgba(152,255,171,0.18); color: #98FFAB; border: 1px solid rgba(152,255,171,0.38); }
.tag-코멧 { background: rgba(255,157,245,0.18); color: #FF9DF5; border: 1px solid rgba(255,157,245,0.38); }

.custom-table { width: 100%; border-collapse: collapse; font-size: 13px; color: #E0D8F8; }
.custom-table thead tr { border-bottom: 1px solid rgba(196,143,255,0.28); }
.custom-table th { padding: 10px 8px; text-align: left; font-weight: 700; color: #C4A0FF; font-size: 12px; letter-spacing: 0.5px; }
.custom-table tbody tr { border-bottom: 1px solid rgba(255,255,255,0.05); transition: background 0.15s; }
.custom-table tbody tr:hover { background: rgba(125,66,255,0.10); }
.custom-table td { padding: 9px 8px; vertical-align: middle; }

.bottom-title { height: 42px; display: flex; align-items: center; margin: 0 0 14px 0; font-size: 26px; font-weight: 950; color: #FFF9FF; letter-spacing: -0.5px; }

div[data-testid="stPlotlyChart"] { background: #15102F !important; border-radius: 24px !important; overflow: hidden !important; padding: 0 !important; box-shadow: 0 0 24px rgba(125,66,255,0.12), inset 0 0 18px rgba(255,255,255,0.018); }
div[data-testid="stPlotlyChart"] > div, .js-plotly-plot, .plot-container, .svg-container { background: transparent !important; border-radius: inherit !important; overflow: hidden !important; }

.dashboard-hero { position: relative; z-index: 3; margin-top: 8px; margin-bottom: 34px; }
.hero-top-row { display: flex; justify-content: space-between; align-items: flex-start; gap: 32px; }
.hero-main-title { font-size: 72px; font-weight: 950; letter-spacing: 11px; line-height: 0.95; color: #FFF8FF; text-shadow: 0 0 6px rgba(255,255,255,0.45), 0 0 16px rgba(198,168,255,0.45), 0 0 32px rgba(125,66,255,0.38); margin: 0 0 14px 0; }
.hero-main-subtitle { font-size: 17px; font-weight: 750; color: #E8DFFF; text-shadow: 0 0 10px rgba(198,168,255,0.18); margin-top: 6px; word-break: keep-all; }
.hero-right-area { width: 560px; display: flex; flex-direction: column; align-items: flex-end; margin-top: 20px; }
.hero-right-area .hero-date-pill { margin-top: 0; margin-bottom: 12px; margin-left: 0; }
.hero-right-area .hero-info-card { width: 100%; margin-top: 0; }
.hero-date-pill { display: inline-flex; align-items: center; gap: 8px; margin-top: 20px; padding: 9px 17px; border-radius: 999px; background: #081A2B !important; border: 1px solid rgba(117,204,255,0.34); color: #D9C8FF; font-size: 14px; font-weight: 850; box-shadow: 0 0 16px rgba(125,66,255,0.14); }
.hero-info-card { width: 600px; min-height: 92px; border-radius: 22px; background: rgba(21,16,47,0.72); border: 1px solid rgba(196,143,255,0.28); box-shadow: 0 0 20px rgba(125,66,255,0.12), inset 0 0 20px rgba(255,255,255,0.02); padding: 18px 22px; display: flex; align-items: center; gap: 16px; }
.hero-info-icon { min-width: 50px; width: 50px; height: 50px; border-radius: 50%; background: radial-gradient(circle at 35% 30%, rgba(255,255,255,0.85), transparent 15%), linear-gradient(135deg, #9B6BFF, #5B2AD8); box-shadow: 0 0 14px rgba(155,107,255,0.38), inset 0 0 10px rgba(255,255,255,0.14); display: flex; align-items: center; justify-content: center; font-size: 21px; }
.hero-info-title { font-size: 15px; font-weight: 900; color: #FFF9FF; margin-bottom: 5px; }
.hero-info-desc { font-size: 12px; line-height: 1.55; color: rgba(255,255,255,0.68); font-weight: 550; word-break: keep-all; }

ul[role="listbox"] { background: #08101F !important; }

.orbit-bg { position: fixed; left: 0; top: 0; right: 0; bottom: 0; z-index: 0; pointer-events: none; opacity: 0.42;
    background: linear-gradient(150deg, transparent 15%, rgba(157,88,255,0.13) 15.4%, transparent 16.5%),
...
@keyframes drift { from { transform: translateY(0px); opacity: 0.28; } to { transform: translateY(-18px); opacity: 0.48; } }
</style>
"""), unsafe_allow_html=True)
...
if st.session_state.page == "대시보드 홈":
    ...
    # -------------------------------------------------------
    # Star Trail 세션 상태 초기화
    # -------------------------------------------------------
    if "current_seg" not in st.session_state:
        st.session_state.current_seg = "프로토스타"
    ...
    # 데이터 로드
    # -------------------------------------------------------
    _st_raw         = load_startrail_raw()
    _st_summary     = load_startrail_summary()
    _st_kpi         = load_startrail_kpi()
    _st_df          = load_startrail_candidates(_st_raw)
    _st_const_df    = load_startrail_constellation()

    # -------------------------------------------------------
    # 아이콘 이미지 (assets/ 없으면 투명 1px)
    # -------------------------------------------------------
    _date_icon  = _img_to_base64("assets/달력.png")
    _soop_icon  = _img_to_base64("assets/숲3.png")
    _chzzk_icon = _img_to_base64("assets/치지직3.png")
    ...
    <div class="hero-main-subtitle">기존 플랫폼의 데이터 궤적을 따라 CIME 영입 후보군을 찾습니다</div>
    </div>
    <div class="hero-right-area">
    <div class="hero-date-pill">
    <img src="data:image/png;base64,{_date_icon}" style="width:16px; height:16px; object-fit:contain;">
    ...
    <div class="hero-info-desc">기존 플랫폼에서 이미 활동성과 팬덤이 확인된 스트리머를 대상으로 방송화력, 수익성, 외부유입 가능성을 함께 검토해 CIME 영입 후보를 선별합니다.</div>
    </div>
    </div>
    </div>
    </div>
    ...
    if _st_kpi:
        ...
        with k1:
            st.metric("총 분석 스트리머 수", f"{_st_kpi.get('총 분석 스트리머 수', 0):,.0f} 명")
        with k2:
            st.metric("평균 뷰어십", f"{_st_kpi.get('평균 뷰어십', 0):,.0f}")
        with k3:
            st.metric("평균 도네이션", f"₩ {_st_kpi.get('평균 도네이션', 0):,.0f}")
    else:
        st.info("KPI 데이터가 없습니다. `10_dashboard/data/startrail_핵심KPI.csv`를 확인해주세요.")
    ...
    _seg_data = {
        "성단":     {"icon": "fa-solid fa-users",   "count": f"{_st_summary.get('성단 그룹 후보 수', 0):,.0f}개",   "desc": "팬덤이 함께 이동할\n그룹형 후보군"},
        "프로토스타":{"icon": "fa-solid fa-leaf",    "count": f"{_st_summary.get('프로토스타 S급 후보 수', 0):,.0f}명","tooltip": "S급 기준", "desc": "성장 반응이 좋은\n성장형 후보군"},
        "위성":     {"icon": "fa-solid fa-satellite","count": f"{_st_summary.get('위성 후보 수', 0):,.0f}명",         "desc": "검증된 개인 방송\n개인형 후보군"},
        "슈퍼노바": {"icon": "fa-solid fa-star",     "count": f"{_st_summary.get('슈퍼노바 핵심 후보 수', 0):,.0f}명","tooltip": "핵심 후보군 기준", "desc": "대중성·팬덤 규모\n간판형 후보군"},
        "코멧":     {"icon": "fa-solid fa-meteor",  "count": f"{_st_summary.get('코멧 후보 수', 0):,.0f}명",         "desc": "방송 외 인지도\n발견형 후보군"},
    }

    _seg_cols = st.columns(5)
    for i, (seg, info) in enumerate(_seg_data.items()):
        with _seg_cols[i]:
            active_class = f"active active-{seg}" if st.session_state.current_seg == seg else ""
            tooltip_html = (
                f'<span class="tooltip-wrap"><span class="tooltip-icon">?</span>'
                f'<span class="tooltip-text">{info["tooltip"]}</span></span>'
            ) if "tooltip" in info else ""
            st.markdown(
                f'<div class="segment-card {active_class}">'
                f'<i class="{info["icon"]} fa-2x"></i>'
                f'<div class="seg-name">{seg}{tooltip_html}</div>'
                f'<div class="seg-count">{info["count"]}</div>'
                f'<div class="seg-desc">{info["desc"]}</div>'
                f'</div>',
                unsafe_allow_html=True,
            )
            if st.button("선택", key=f"st_seg_btn_{seg}", use_container_width=True):
                st.session_state.current_seg = seg
                st.session_state.show_sidebar = False
                st.session_state.selected_streamer = None
                st.rerun()
    ...
    # -------------------------------------------------------
    _platform_filter = "전체"
    _seg_detail_filter = "전체"

    if st.session_state.current_seg == "성단":
        pass
    elif st.session_state.current_seg == "코멧":
        _, f1, f2 = st.columns([2, 1, 1])
        _platform_filter   = f1.selectbox("플랫폼 필터", ["전체", "SOOP", "CHZZK"], key="st_plat_f")
        _seg_detail_filter = f2.selectbox("세그먼트 필터", ["전체", "X 강세형", "유튜브 강세형", "하이브리드"], key="st_seg_f")
    else:
        _, _, f1 = st.columns([2, 1, 1])
        _platform_filter = f1.selectbox("플랫폼 필터", ["전체", "SOOP", "CHZZK"], key="st_plat_f2")

    # -------------------------------------------------------
    # 데이터 필터링
    # -------------------------------------------------------
    if st.session_state.current_seg == "성단":
        _filtered_df = _st_const_df.copy() if not _st_const_df.empty else pd.DataFrame()
    else:
        _filtered_df = _st_df[_st_df["세그먼트"] == st.session_state.current_seg].copy() if not _st_df.empty else pd.DataFrame()
        if _platform_filter != "전체" and not _filtered_df.empty:
            _filtered_df = _filtered_df[_filtered_df["플랫폼"] == _platform_filter].copy()
        if _seg_detail_filter != "전체" and not _filtered_df.empty:
            _filtered_df = _filtered_df[_filtered_df["세그먼트필터"] == _seg_detail_filter].copy()

    if not _filtered_df.empty:
        _filtered_df = _filtered_df.sort_values("스코어", ascending=False).reset_index(drop=True)
        _filtered_df["순위"] = _filtered_df.index + 1
        _filtered_df["상위퍼센트"] = (_filtered_df["순위"] / len(_filtered_df) * 100).round(2)

    _top5 = _filtered_df.head(5) if not _filtered_df.empty else pd.DataFrame()
    ...
    if _top5.empty:
        st.warning("선택한 조건에 해당하는 후보가 없습니다. CSV 파일을 확인해주세요.")
    else:
        _card_cols = st.columns(5)
        for i, (_, row) in enumerate(_top5.iterrows()):
            if st.session_state.current_seg == "성단":
                _dname   = row["소속"]
                _dseg    = "성단"
                _score_t = f'{row["스코어"]:.0f}'
            else:
                _dname   = row["스트리머"]
                _dseg    = row["세그먼트"]
                _score_t = f'{row["스코어"]:.2f}'

            _img_url = str(row.get("이미지URL", "")).strip()
            _avatar  = _img_url if (_img_url and _img_url.lower() != "nan") else f"https://api.dicebear.com/7.x/avataaars/svg?seed={_dname}"

            _rank_cls = {0: "rank-gold", 1: "rank-silver", 2: "rank-bronze"}.get(i, "rank-normal")

            with _card_cols[i]:
                st.markdown(
                    f'<div class="rank-card">'
                    f'<div class="rank-badge {_rank_cls}">{i+1}</div>'
                    f'<div class="avatar-circle"><img src="{_avatar}" alt="avatar"></div>'
                    f'<div style="font-weight:bold;font-size:18px;">{_dname}</div>'
                    f'<div class="tag-segment tag-{_dseg}">{_dseg}</div>'
                    f'<div style="margin-top:15px;font-size:12px;color:#8b949e;">스코어</div>'
                    f'<div style="font-size:24px;font-weight:800;color:#ffbc00;">{_score_t}</div>'
                    f'</div>',
                    unsafe_allow_html=True,
                )
                if st.button("상세 보기", key=f"st_card_{_dseg}_{_dname}_{i}", use_container_width=True):
                    _sel = row.to_dict()
                    _sel["세그먼트"] = st.session_state.current_seg
                    _sel["표시이름"] = _dname
                    st.session_state.selected_streamer = _sel
    ...
    if st.session_state.show_sidebar and st.session_state.selected_streamer is not None:
        ...
        _detail_name = s.get("표시이름", s.get("스트리머", "-"))
        _img_url     = str(s.get("이미지URL", "")).strip()
        _avatar_url  = _img_url if (_img_url and _img_url.lower() != "nan" and s.get("세그먼트") != "성단") else f"https://api.dicebear.com/7.x/avataaars/svg?seed={_detail_name}"

        # 플랫폼 아이콘
        if s.get("세그먼트") == "성단":
            _plat_html = ""
        elif s.get("플랫폼") == "SOOP":
            _plat_html = f'<span style="display:flex;align-items:center;height:30px;"><img src="data:image/png;base64,{_soop_icon}" style="height:28px;border-radius:8px;object-fit:contain;"></span>'
        elif s.get("플랫폼") == "CHZZK":
            _plat_html = f'<span style="display:flex;align-items:center;height:30px;"><img src="data:image/png;base64,{_chzzk_icon}" style="height:28px;border-radius:8px;object-fit:contain;"></span>'
        else:
            _plat_html = f'<span style="color:#8b949e;font-size:14px;">{s.get("플랫폼","")}</span>'

        # 지표
        if s.get("세그먼트") == "성단":
            _const_ref = _st_const_df if not _st_const_df.empty else pd.DataFrame({"멤버수":[1],"합계_뷰어십":[1],"합계_도네이션":[1]})
            _metrics = {
                "멤버 수":      (s.get("멤버수", 0),       max(_const_ref["멤버수"].quantile(0.95), 1),       "#00ff87"),
                "뷰어십 합계":  (s.get("합계_뷰어십", 0),  max(_const_ref["합계_뷰어십"].quantile(0.95), 1),  "#ff4b4b"),
                "도네이션 합계":(s.get("합계_도네이션", 0), max(_const_ref["합계_도네이션"].quantile(0.95), 1),"#ffbc00"),
            }
            _radar_metrics = {
                "플랫폼체급": (s.get("플랫폼체급_점수", 0), 100),
                "ARPU":      (s.get("ARPU_점수", 0),       100),
                "외부인기":   (s.get("외부인기_점수", 0),   100),
                "대중성":     (s.get("대중성_점수", 0),     100),
            }
        else:
            _metrics = {
                "뷰어십":    (s.get("뷰어십", 0),    17000000, "#ff4b4b"),
                "도네이션":  (s.get("도네이션", 0),  7000000,  "#ffbc00"),
                "최고 팔로워":(s.get("팔로워수", 0), 414000,   "#00ff87"),
                "평균 시청자":(s.get("평균시청자", 0),47500,   "#00f2ff"),
                "최고 시청자":(s.get("최고시청자", 0),50000,   "#bf40bf"),
            }
            _radar_metrics = {
                "대중성":       (s.get("대중성_표준점수", 0),       100),
                "방송화력":     (s.get("방송화력_표준점수", 0),     100),
                "팬덤결집력":   (s.get("팬덤결집력_표준점수", 0),   100),
                "수익성":       (s.get("수익성_표준점수", 0),       100),
                "외부유입가능성":(s.get("외부유입가능성_표준점수", 0),100),
            }

        st.markdown("<div style='height:20px;'></div>", unsafe_allow_html=True)
        with st.container(border=True):
            st.markdown("<div style='height:30px;'></div>", unsafe_allow_html=True)
            _dl, _dm, _dr = st.columns([1.1, 1.8, 1.2])

            with _dl:
                st.markdown(
                    f'<div style="width:190px;margin:22px auto 0 auto;text-align:center;">'
                    f'<div class="avatar-circle" style="width:135px;height:135px;border:4px solid #00f2ff;margin:0 auto 14px auto;">'
                    f'<img src="{_avatar_url}" alt="avatar" style="width:100%;height:100%;object-fit:cover;"></div>'
                    f'<div style="width:190px;text-align:center;font-size:26px;font-weight:800;color:white;margin:0 0 6px 0;white-space:nowrap;">{_detail_name}</div>'
                    f'<div style="width:190px;display:flex;justify-content:center;gap:8px;align-items:center;">'
                    f'<span class="tag-segment tag-{s.get("세그먼트","")}" style="height:30px;padding:0 12px;display:flex;align-items:center;justify-content:center;margin:0;">{s.get("세그먼트","")}</span>'
                    f'{_plat_html}</div></div>',
                    unsafe_allow_html=True,
                )

            with _dm:
                for m_name, (val, max_val, color) in _metrics.items():
                    pct = min(val / max(max_val, 1), 1.0) * 100
                    st.markdown(
                        f'<div style="margin-bottom:14px;">'
                        f'<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px;">'
                        f'<span style="font-size:14px;font-weight:bold;color:white;">{m_name}</span>'
                        f'<span style="font-size:15px;font-weight:800;color:{color};">{val:,.0f}</span></div>'
                        f'<div style="background:rgba(255,255,255,0.05);height:8px;border-radius:4px;overflow:hidden;">'
                        f'<div style="background:{color};height:100%;border-radius:4px;width:{pct}%;"></div></div></div>',
                        unsafe_allow_html=True,
                    )

            with _dr:
                _r_labels = list(_radar_metrics.keys())
                _r_values = [min(v / max(m, 1), 1.0) * 100 for v, m in _radar_metrics.values()]
                _fig_radar = go.Figure()
                _fig_radar.add_trace(go.Scatterpolar(
                    r=_r_values + [_r_values[0]],
                    theta=_r_labels + [_r_labels[0]],
                    fill="toself", name="능력치",
                    line=dict(width=2), opacity=0.85,
                ))
                _fig_radar.update_layout(
                    polar=dict(bgcolor=PLOTLY_TRANSPARENT,
                    ...
                    showlegend=False, margin=dict(l=35,r=35,t=25,b=45),
                    paper_bgcolor=PLOTLY_TRANSPARENT, plot_bgcolor=PLOTLY_TRANSPARENT,
                    font_color="white", height=260,
                )
                st.plotly_chart(_fig_radar, use_container_width=True)

            _, _close_col = st.columns([5, 1])
            with _close_col:
                if st.button("닫기", key="st_detail_close", use_container_width=True):
                    st.session_state.show_sidebar = False
                    st.session_state.selected_streamer = None
                    st.rerun()
    ...
    # -------------------------------------------------------
    st.write("---")
    _bl, _sp, _br = st.columns([1, 0.08, 1])

    with _bl:
        st.markdown(f"<div class='bottom-title'>📋 {st.session_state.current_seg} 영입 우선순위 리스트</div>", unsafe_allow_html=True)
        if _filtered_df.empty:
            st.info("표시할 데이터가 없습니다.")
        else:
            if st.session_state.current_seg == "성단":
                _thtml = "<table class='custom-table'><thead><tr><th>순위</th><th>소속</th><th>스코어</th><th>상위 %</th><th>멤버 수</th><th>뷰어십 합계</th><th>도네이션 합계</th></tr></thead><tbody>"
                for _, row in _filtered_df.head(10).iterrows():
                    _thtml += (f"<tr><td>{row['순위']}</td><td><b>{row['소속']}</b></td>"
                               f"<td style='color:#00f2ff;font-weight:bold;'>{row['스코어']:.0f}</td>"
                               f"<td style='color:#ffbc00;font-weight:bold;'>상위 {row['상위퍼센트']:.1f}%</td>"
                               f"<td>{row.get('멤버수',0):,.0f}</td><td>{row.get('합계_뷰어십',0):,.0f}</td>"
                               f"<td>₩ {row.get('합계_도네이션',0):,.0f}</td></tr>")
            else:
                _thtml = "<table class='custom-table'><thead><tr><th>순위</th><th>스트리머명</th><th>플랫폼</th><th>스코어</th><th>상위 %</th><th>팔로워수</th><th>평균시청자</th><th>평균 도네이션</th></tr></thead><tbody>"
                for _, row in _filtered_df.head(10).iterrows():
                    if row["플랫폼"] == "SOOP":
                        _p = f'<img src="data:image/png;base64,{_soop_icon}" style="height:24px;border-radius:8px;object-fit:contain;">'
                    elif row["플랫폼"] == "CHZZK":
                        _p = f'<img src="data:image/png;base64,{_chzzk_icon}" style="height:24px;border-radius:8px;object-fit:contain;">'
                    else:
                        _p = row["플랫폼"]
                    _thtml += (f"<tr><td>{row['순위']}</td><td><b>{row['스트리머']}</b></td><td>{_p}</td>"
                               f"<td style='color:#00f2ff;font-weight:bold;'>{row['스코어']:.2f}</td>"
                               f"<td style='color:#ffbc00;font-weight:bold;'>상위 {row['상위퍼센트']:.1f}%</td>"
                               f"<td style='text-align:right;'>{row.get('팔로워수',0):,.0f}</td>"
                               f"<td style='text-align:right;'>{row.get('평균시청자',0):,.0f}</td>"
                               f"<td style='text-align:right;'>₩ {row.get('평균도네이션',0):,.0f}</td></tr>")
            _thtml += "</tbody></table>"
            st.markdown(_thtml, unsafe_allow_html=True)

    with _br:
        if st.session_state.current_seg == "성단":
            _gtitle = "🌌 성단 TOP15 히트맵"
        elif st.session_state.current_seg == "코멧":
            _gtitle = "🌌 코멧 타겟팅 맵"
        else:
            _gtitle = f"🌌 {st.session_state.current_seg} 세그먼트 분석 분포"
        st.markdown(f"<div class='bottom-title'>{_gtitle}</div>", unsafe_allow_html=True)

        if _filtered_df.empty:
            st.warning("그래프를 표시할 데이터가 없습니다.")

        elif st.session_state.current_seg == "성단":
            _hm_cols = ["플랫폼체급_점수","ARPU_점수","외부인기_점수","대중성_점수","영입우선_점수"]
            _hm_labels = ["플랫폼체급","ARPU","외부인기","대중성","영입우선"]
            _avail = [c for c in _hm_cols if c in _filtered_df.columns]
            if _avail:
                _hm_df = _filtered_df.sort_values("영입우선_점수", ascending=False).head(15).set_index("소속")[_avail]
                _fig = px.imshow(_hm_df, text_auto=".0f", aspect="auto", color_continuous_scale="YlGnBu", zmin=0, zmax=100, labels=dict(color="점수"))
                _fig.update_xaxes(ticktext=_hm_labels[:len(_avail)], tickvals=list(range(len(_avail))), tickangle=0, automargin=True)
                _fig.update_yaxes(automargin=True)
                _fig.update_layout(paper_bgcolor=PLOTLY_TRANSPARENT, plot_bgcolor=PLOTLY_TRANSPARENT, font_color="white", margin=dict(l=85,r=95,t=55,b=95), height=620, coloraxis_colorbar=dict(title="점수",x=1.03,xanchor="left"))
                st.plotly_chart(_fig, use_container_width=True)
            else:
                st.info("히트맵에 필요한 점수 컬럼이 없습니다.")

        elif st.session_state.current_seg == "코멧":
            _all = _st_raw.copy() if not _st_raw.empty else pd.DataFrame()
            if not _all.empty:
                for _c in ["유튜브_구독자","X_팔로워","최고_팔로워"]:
                    _all[_c] = pd.to_numeric(_all.get(_c, 0), errors="coerce").fillna(0)
                _all["통합_외부화력"] = _all["유튜브_구독자"] + _all["X_팔로워"]
                _plot_all = _all[(_all["최고_팔로워"]>0)&(_all["통합_외부화력"]>0)].copy()
                _plot_comet = _filtered_df.copy()
                _plot_comet["통합_외부화력"] = pd.to_numeric(_plot_comet.get("유튜브_구독자",0),errors="coerce").fillna(0) + pd.to_numeric(_plot_comet.get("X_팔로워",0),errors="coerce").fillna(0)
                _plot_comet["최고_팔로워"] = pd.to_numeric(_plot_comet.get("최고_팔로워",0),errors="coerce").fillna(0)
                _plot_comet = _plot_comet[(_plot_comet["최고_팔로워"]>0)&(_plot_comet["통합_외부화력"]>0)].copy()
                _fig = go.Figure()
                _fig.add_trace(go.Scatter(x=_plot_all["최고_팔로워"], y=_plot_all["통합_외부화력"], mode="markers", name="일반 스트리머", marker=dict(size=6,color="rgba(217,217,217,0.35)"), text=_plot_all.get("스트리머명",""), hovertemplate="<b>%{text}</b><br>최고 팔로워: %{x:,.0f}<br>통합 외부화력: %{y:,.0f}<extra></extra>"))
                for _route, _color in {"유튜브 강세형":"#FF8A8A","X 강세형":"#95AFFF","하이브리드":"#FF9DF5"}.items():
                    _tmp = _plot_comet[_plot_comet.get("코멧유입경로","") == _route] if "코멧유입경로" in _plot_comet.columns else pd.DataFrame()
                    if not _tmp.empty:
                        _fig.add_trace(go.Scatter(x=_tmp["최고_팔로워"], y=_tmp["통합_외부화력"], mode="markers", name=_route, marker=dict(size=14,color=_color,line=dict(color="white",width=1.5)), text=_tmp.get("스트리머",""), hovertemplate=f"<b>%{{text}}</b><br>유입경로: {_route}<br>최고 팔로워: %{{x:,.0f}}<br>통합 외부화력: %{{y:,.0f}}<extra></extra>"))
                if not _plot_comet.empty:
                    _avg = _plot_comet["최고_팔로워"].mean()
                    _fig.add_vline(x=_avg, line_dash="dot", line_width=2, line_color="#c9d1d9", opacity=0.75)
                    _fig.add_annotation(x=_avg, y=_plot_all["통합_외부화력"].max(), text=f"코멧 평균 최고 팔로워<br>{_avg:,.0f}명", showarrow=False, xanchor="left", yanchor="top", font=dict(size=11,color="#c9d1d9"))
                _fig.update_layout(title=dict(text="외부 팬덤 vs 방송 체급",x=0.08,xanchor="left"), xaxis_title="방송 체급", yaxis_title="통합 외부 화력", xaxis_type="log", yaxis_type="log", paper_bgcolor=PLOTLY_TRANSPARENT, plot_bgcolor=PLOTLY_TRANSPARENT, font_color="white", margin=dict(l=80,r=140,t=75,b=95), height=620, legend=dict(title="코멧 유입경로",bgcolor=PLOTLY_TRANSPARENT,x=1.03,y=0.98,xanchor="left",yanchor="top"))
                _fig.update_xaxes(gridcolor="rgba(255,255,255,0.12)",automargin=True)
                _fig.update_yaxes(gridcolor="rgba(255,255,255,0.12)",automargin=True)
                st.plotly_chart(_fig, use_container_width=True)
            else:
                st.info("코멧 그래프에 필요한 원본 데이터가 없습니다.")

        else:
            _req = {"평균시청자", "스코어", "뷰어십", "플랫폼"}
            if _req.issubset(set(_filtered_df.columns)):
                _fig = px.scatter(_filtered_df, x="평균시청자", y="스코어", size="뷰어십", color="플랫폼",
                                  hover_name="스트리머", color_discrete_map={"SOOP":"#00f2ff","CHZZK":"#bf40bf"}, template="plotly_dark")
                _fig.update_layout(paper_bgcolor=PLOTLY_TRANSPARENT, plot_bgcolor=PLOTLY_TRANSPARENT, margin=dict(l=85,r=170,t=65,b=95), height=620, font_color="white", legend=dict(bgcolor=PLOTLY_TRANSPARENT,x=1.03,y=0.98,xanchor="left",yanchor="top"))
                _fig.update_xaxes(automargin=True, gridcolor="rgba(255,255,255,0.12)")
                _fig.update_yaxes(automargin=True, gridcolor="rgba(255,255,255,0.12)")
                st.plotly_chart(_fig, use_container_width=True)
            else:
                st.info("산점도에 필요한 컬럼이 부족합니다.")

    st.stop()

# =========================================================
# 4-B. Star Trail 데이터 경로 및 로더 (팀원 파일 통합)
# =========================================================

STARTRAIL_SUMMARY_PATH      = resolve_existing_path("10_dashboard/data/startrail_대시보드요약.csv")
STARTRAIL_KPI_PATH          = resolve_existing_path("10_dashboard/data/startrail_핵심KPI.csv")
STARTRAIL_CANDIDATE_PATH    = resolve_existing_path("10_dashboard/data/startrail_개인후보통합테이블.csv")
STARTRAIL_CONSTELLATION_PATH= resolve_existing_path("10_dashboard/data/startrail_성단그룹후보테이블.csv")
...
def _img_to_base64(path: str) -> str:
    """assets/ 이미지를 base64로 변환. 없으면 1px 투명 PNG 반환."""
    try:
        full = resolve_existing_path(path)
...
def load_startrail_summary():
    if not STARTRAIL_SUMMARY_PATH.exists():
        return {}
    for enc in ["utf-8-sig", "cp949", "utf-8"]:
        try:
            df = pd.read_csv(STARTRAIL_SUMMARY_PATH, encoding=enc)
            return dict(zip(df["지표"], df["값"]))
        except Exception:
            continue
    return {}
...
@st.cache_data(show_spinner=False)
def load_startrail_kpi():
    if not STARTRAIL_KPI_PATH.exists():
        return {}
    for enc in ["utf-8-sig", "cp949", "utf-8"]:
        try:
            df = pd.read_csv(STARTRAIL_KPI_PATH, encoding=enc)
            return dict(zip(df["지표"], df["값"]))
        except Exception:
            continue
    return {}
...
@st.cache_data(show_spinner=False)
def load_startrail_raw():
    if not STARTRAIL_CANDIDATE_PATH.exists():
        return pd.DataFrame()
    for enc in ["utf-8-sig", "cp949", "utf-8"]:
        try:
            return pd.read_csv(STARTRAIL_CANDIDATE_PATH, encoding=enc, low_memory=False)
        except Exception:
            continue
    return pd.DataFrame()
...
@st.cache_data(show_spinner=False)
def load_startrail_candidates(_raw: pd.DataFrame) -> pd.DataFrame:
    if _raw.empty:
        return pd.DataFrame()
    raw = _raw.copy()
    data_list = []
    ...
    for seg, rule in segment_rules.items():
        fc, fv, sc = rule["filter_col"], rule["filter_value"], rule["score_col"]
        if fc not in raw.columns or sc not in raw.columns:
            continue
        if seg == "코멧":
            temp = raw[pd.to_numeric(raw[fc], errors="coerce").fillna(0) == 1].copy()
        else:
            temp = raw[raw[fc] == fv].copy()
        if temp.empty:
            continue
        temp["세그먼트"] = seg
        temp["스코어"] = pd.to_numeric(temp[sc], errors="coerce").fillna(0).round(2)
    ...
    df["스트리머"] = df["스트리머명"]
    df["이미지URL"] = df.get("이미지URL", pd.Series(dtype=str)).fillna("").astype(str).str.strip() if "이미지URL" in df.columns else ""
...
@st.cache_data(show_spinner=False)
def load_startrail_constellation() -> pd.DataFrame:
    if not STARTRAIL_CONSTELLATION_PATH.exists():
        return pd.DataFrame()
    for enc in ["utf-8-sig", "cp949", "utf-8"]:
        try:
            const = pd.read_csv(STARTRAIL_CONSTELLATION_PATH, encoding=enc, low_memory=False)
            const["소속"] = const["소속"].astype(str).str.strip()
            const["스코어"] = pd.to_numeric(const["영입우선_점수"], errors="coerce").fillna(0).round(0)
            for col in ["멤버수", "합계_뷰어십", "합계_도네이션"]:
                if col in const.columns:
                    const[col] = pd.to_numeric(const[col], errors="coerce").fillna(0)
            const = const.sort_values("스코어", ascending=False).reset_index(drop=True)
            const["순위"] = const.index + 1
            const["상위퍼센트"] = (const["순위"] / max(len(const), 1) * 100).round(2) if len(const) > 0 else 0
            return const
        except Exception:
            continue
    return pd.DataFrame()
